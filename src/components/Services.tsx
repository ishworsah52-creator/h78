            ...
            filteredServices.map((service) => (
              ...
              >
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                  <a
                    href={`https://wa.me/977${BUSINESS_INFO.phone}?text=${encodeURIComponent(`Hello h78nepal 💕, I am interested in booking: ${service.name}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 rounded-lg transition-colors"
                    title={`WhatsApp chat for ${service.name}`}
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-pink-600 hover:bg-pink-700 px-3 py-1.5 rounded-lg transition-colors shadow-sm"
                  >
